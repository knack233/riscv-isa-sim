...
void DifftestRef::update_mip(void *non_reg_interrupt_pending) {
  ...
  state->mip->backdoor_write_with_mask(MIP_STIP, n->platform_irp_stip ? MIP_STIP : 0);
}
...
void DifftestRef::display() {
  ...
  auto& vstate = p->VU;
  for(i = 0; i < 32; i ++) {
    printf("%4s_0: " FMT_WORD " ", vr_name[i], vstate.elt<uint64_t>(i, 0,false));
    printf("%4s_1: " FMT_WORD " ", vr_name[i], vstate.elt<uint64_t>(i, 1,false));
    if (i % 4 == 3) {
      printf("\n");
    }
  }
  printf("vstart: " FMT_WORD " vxsat: " FMT_WORD " vxrm: " FMT_WORD " vcsr: " FMT_WORD "\n",vstate.vstart->read(),
    vstate.vxsat->read(), vstate.vxrm->read(),state->csrmap[CSR_VCSR]->read());
  printf("vl: " FMT_WORD " vtype :" FMT_WORD " vlenb: " FMT_WORD "\n", vstate.vl->read(),vstate.vtype->read(),vstate.vlenb);
